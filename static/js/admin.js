...
async function sendMessage(event = new CustomEvent('click')) {
    ...
    if (messageText !== '') {
        ...
        try { 
            ...
            let response = await fetch(window.location.origin + '/api/admin?' + qr, { method: "post" });
...
window.addEventListener('DOMContentLoaded', async function () {
    let responses = await fetch(window.location.origin + '/api/admin');
...
async function setUpNotifications() {
    try {
        if ('serviceWorker' in navigator) {
            ...
            let response =await this.fetch(this.window.location.origin + '/api/notice/admin/subscribe', {
                method: 'POST',
